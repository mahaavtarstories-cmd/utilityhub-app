...
export default async function SourcesPage() {
  ...
  const { data: sources } = await supabase
    ...
    .select('id, project_id, website_name, url, purpose, status, priority, is_global, projects(name, platform)')
  ...
  return (
    <div className="space-y-6">
      ...
      {globalSources.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-3">🌍 Shared Across All Projects ({globalSources.length})</h2>
              ...
              <tbody>
                {globalSources.map((s: any) => (
                  <tr key={s.id} className="border-t border-slate-700 hover:bg-slate-700/30">
                    ...
                    <td className="px-4 py-3"><span className={`text-xs ${statusColors[s.status] || 'text-gray-400'}`}>● {s.status.replace('_', ' ')}</span></td>
                  </tr>
                ))}
              </tbody>
            </table>
      ...
      {/* Per-project cards */}
      <div>
        <h2 className="text-lg font-semibold text-white mb-3">Per-Project Sources</h2>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-3">
          {projects?.map((p: any) => {
            const projectSourcesList = projectSources.filter((s: any) => s.project_id === p.id)
            const approvedCount = projectSourcesList.filter((s: any) => s.status === 'approved').length
            return (
              <Link key={p.id} href={`/app/projects/${p.id}/sources`}
                ...
                <p className="text-xs text-slate-400 mt-1">{approvedCount} approved / {projectSourcesList.length} total</p>
              </Link>
            )
          })}
        </div>
      ...
      {/* All project-specific sources table */}
      {projectSources.length > 0 && (
        <div>
          <h2 className="text-lg font-semibold text-white mb-3">All Project-Specific Sources</h2>
